...
//    self.animation_line.backgroundColor = [UIColor redColor];
    ...
    self.timer =[NSTimer timerWithTimeInterval:timer_animation_Duration target:self selector:@selector(animation_line_action) userInfo:nil repeats:YES];
...
#pragma mark - - - 扫描外部View的创建
    ...
    butten.frame = CGRectMake(10*Kscr, KscrH-50*Kscr, 30*Kscr, 30*Kscr);
    [butten addTarget:self action:@selector(albumBtnclic) forControlEvents:UIControlEventTouchUpInside];
    butten.backgroundColor = [UIColor redColor];
    [self addSubview:butten];
    //添加取消按钮
    UIButton *dissbutten = [[UIButton alloc]init];
    dissbutten.frame = CGRectMake(KscrW -40*Kscr, 40*Kscr, 30*Kscr, 30*Kscr);
    
    [dissbutten addTarget:self action:@selector(dissBtn)forControlEvents:UIControlEventTouchUpInside];
    dissbutten.backgroundColor = [UIColor redColor];
    [self addSubview:dissbutten];
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    right_imageView_down.layer.borderColor = [UIColor redColor].CGColor;
...
-(void)albumBtnclic
{
    [self.deleget alumBtnClic];
}
-(void)dissBtn
{
    [self.deleget dissMissBtnClic];
}
@end
