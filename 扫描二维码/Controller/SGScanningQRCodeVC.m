...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.scanningView.deleget = self;
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    for (int index = 0; index < [features count]; index ++) {
        ...
        if (self.first_push) {
            ...
            self.ResulestBlock(scannedResult);
...
//    [self playSoundEffect:@"sound.caf"];
    ...
    if (metadataObjects.count > 0) {
        ...
        if ([obj.stringValue hasPrefix:@"http"]) {
            ...
            self.ResuleUrlBlock(obj.stringValue);
//            ScanSuccessJumpVC *jumpVC = [[ScanSuccessJumpVC alloc] init];
...
//            [self.navigationController pushViewController:jumpVC animated:YES];
            ...
            self.ResulestBlock(obj.stringValue);
...
#pragma mark----点击相册和取消按钮
-(void)alumBtnClic
{
    [self rightBarButtonItenAction];
}
-(void)dissMissBtnClic
{
    [self dismissViewControllerAnimated:NO completion:nil];
}
